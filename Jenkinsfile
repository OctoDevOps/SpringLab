...
//        dir('SpringLab') {
        ...
        stage("Build with Unit Testing ")
        {
                    ...
                    withSonarQubeEnv('localhost_sonarqube') {
                        sh "pwd;'${mvnHome}/bin/mvn' -X -Dintegration-tests.skip=trucleafite -Dbuild.number=${targetVersion} clean verify sonar:sonar -Dsonar.sources=src/main/java -Dsonar.jacoco.reportPath=target/jacoco.exec -Dsonar.tests=src/test/java"
                    }
