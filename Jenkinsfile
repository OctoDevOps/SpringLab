...
//        dir('SpringLab') {
        ...
        stage("Build with Unit Testing ")
        {
                    ...
                    withSonarQubeEnv('localhost_sonarqube') {
                        sh "pwd;'${mvnHome}/bin/mvn' -Dintegration-tests.skip=true -Dbuild.number=${targetVersion} clean verify sonar:sonar -Dsonar.projectVersion=41 -Dsonar.projectName='JPT Automation' -Dsonar.sources=src/main/java -Dsonar.tests=src/test/java -Dsonar.jacoco.xmlReportPaths=target/jacoco-ut/*.xml -Dsonar.projectKey='JptAutomationProject'"
                    }
