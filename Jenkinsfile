...
//        dir('SpringLab') {
        ...
        stage("Build with Unit Testing ")
        {
                    ...
                    withSonarQubeEnv('localhost_sonarqube') {
                        sh "pwd;'${mvnHome}/bin/mvn' -Dintegration-tests.skip=true -Dbuild.number=${targetVersion} clean install sonar:sonar -Dsonar.projectVersion=41 -Dsonar.projectName='jptAutomation' -Dsonar.sources=src/main/java -Dsonar.tests=src/test/java -Dsonar.jacoco.reportPath=target/jacoco.exec"
                    }
